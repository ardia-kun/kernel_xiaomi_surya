...
# By Fannndi & ChatGPT (Clang Android 15 - Final + Auto mrproper Fix)
set -euo pipefail
...
# ===================== HELPER =====================
require_tools() {
    for tool in git wget tar unzip python3 zip; do
...
ensure_clean_source() {
    echo "==> Checking if source tree is clean..."
    if make O=out ARCH=arm64 CC=clang HOSTCC=clang HOSTCXX=clang++ \
        CLANG_TRIPLE=aarch64-linux-gnu- LD=ld.lld LLVM=1 LLVM_IAS=1 \
        prepare 2>&1 | grep -q "is not clean"; then
        echo "==> Source is dirty. Running mrproper..."
        backup_config
        make mrproper
        restore_config
    fi
...
compile_kernel() {
    ...
    # Cek source kotor sebelum build
    if ! make O=out ARCH=arm64 CC=clang HOSTCC=clang HOSTCXX=clang++ \
        CLANG_TRIPLE=aarch64-linux-gnu- LD=ld.lld LLVM=1 LLVM_IAS=1 prepare; then
        if grep -q "is not clean" log.txt; then
            echo "==> Detected dirty source, running mrproper..."
            backup_config
            make mrproper
            restore_config
            make_defconfig
